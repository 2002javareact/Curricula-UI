...
import { Container, Button, Row, Col, Card, CardTitle, CardText, CardColumns, InputGroup, Input, InputGroupButtonDropdown, DropdownToggle, DropdownMenu, DropdownItem, Label, UncontrolledDropdown, Form, FormGroup, Badge, CardBody } from 'reactstrap';
  ...
  // }
    ...
    /*
      ...
      not existing
        - changes not
    */
  static getDerivedStateFromProps(props:any,state:IViewAndUpdateCurriculumState){
                    ...
                    {this.props.curriculum?(
                        <React.Fragment>
                            ...
                            <Col lg={12}>
                                ...
                                <Form>
              <FormGroup>
                <Label className="text-left">Enter new name of the Curriculum</Label>
            ...
            <Form className="shadow-custom">
              <FormGroup>
                {this.state.existSkillList.length!==0 &&
                  <React.Fragment>
                    <Label>Skills Currently in this Curriculum:</Label>
                    {this.state.existSkillList.map((el:Skill)=>
                      <Card style={{backgroundColor:el.category.categoryColor}}>
                        <CardBody>
                          <CardTitle className="text-light font-weight-bold">{el.skillName}</CardTitle>
                          <Button className="text-light font-weight-bold" color="danger" onClick={(e:SyntheticEvent)=>this.handlerRemoveSkill(e,el)}>Remove</Button>
                        </CardBody>
                      </Card>
                    )}
                  </React.Fragment>
                ...
                {this.state.notExistSkillList.length!==0 &&
                  <React.Fragment>
                    <Label>Skills Currently Not in this Curriculum</Label>
                    {this.state.notExistSkillList.map((el:Skill)=>
                      <Card style={{backgroundColor:el.category.categoryColor}}>
                        <CardBody>
                          <CardTitle className="text-light font-weight-bold">{el.skillName}</CardTitle>
                          <Button className="text-light font-weight-bold" color="primary" onClick={(e:SyntheticEvent)=>this.handlerAddSkill(e,el)}>Add</Button>
                        </CardBody>
                      </Card>
                    )}
                  </React.Fragment>
